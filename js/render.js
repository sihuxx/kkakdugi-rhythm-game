...
function judgeMarks(t,th){
  const beat=((t/song.beat)%1+1)%1;
  const pulse=REDUCED?1:1+Math.max(0,0.12*(1-beat*3));
  [['tap','#7BC47F'],['jump','#A9D9F0']].forEach(([kind,col])=>{
    const y=laneY(kind), r=(kind==='jump'?20:24)*pulse;
    g.save(); g.setLineDash([7,7]); g.lineWidth=3; g.strokeStyle=col;
    g.beginPath(); g.arc(judgeX(),y,r,0,6.3); g.stroke(); g.restore();
  });
}
function drawNotes(t,th){
  ...
  for(const n of notes){
    ...
    const y=laneY(n.type);
    if(n.type==='hold'){
      if(n.done) continue;                          // 끝났으면(완주·놓침) 바로 사라진다
      const tailX=noteX(n.t+n.dur);
      if(tailX<-60) continue;                       // 꼬리까지 지나야 사라진다
      const x1 = n.holding ? jx : noteX(n.t);       // 잡고 있는 동안만 판정선에 고정
      if(x1>tailX) continue;
      g.lineCap='round';
      g.strokeStyle=n.judged?'#D9E8CF':'#F2E8D9'; g.lineWidth=30;
      g.beginPath(); g.moveTo(x1,y); g.lineTo(tailX,y); g.stroke();
      g.strokeStyle=th.ink; g.lineWidth=2.6;
      g.beginPath(); g.moveTo(x1,y-15); g.lineTo(tailX,y-15); g.stroke();
      g.beginPath(); g.moveTo(x1,y+15); g.lineTo(tailX,y+15); g.stroke();
      wobble(tailX,y,9,n.seed,'#F2E8D9',th.ink);
      if(!n.judged) wobble(noteX(n.t),y,17,n.seed,'#D9C4A0',th.ink);
      else if(n.holding){                           // 잡고 있는 동안 손잡이가 반짝
        const p=1+Math.sin(t*22)*0.12;
        wobble(jx,y,15*p,n.seed,'#7BC47F',th.ink);
        if(Math.random()<0.3) fx.push({x:jx,y,life:.6,kind:'dust',vx:(Math.random()-.5)*90,vy:-60-Math.random()*70,r:2+Math.random()*2});
      }
    } else {
      if(n.done) continue;
      const x=noteX(n.t);
      if(x<-60) continue;
      if(n.type==='jump'){
        wobble(x,y,19,n.seed,'#A9D9F0',th.ink);
        g.strokeStyle=th.ink; g.lineWidth=2.5;
        g.beginPath(); g.moveTo(x-11,y+2); g.quadraticCurveTo(x-19,y-4,x-20,y-10); g.stroke();
        g.beginPath(); g.moveTo(x+11,y+2); g.quadraticCurveTo(x+19,y-4,x+20,y-10); g.stroke();
      } else wobble(x,y,16,n.seed,'#7BC47F',th.ink);
    }
  }
}
...
function drawChar(t,dt,th){
  const gy=groundY()-char.float, scale=Math.min(H*0.27, W*0.17)*char.scale;
  let key=char.run, y=gy, rot=0, sc=scale;
  const bob=Math.abs(Math.sin(t*Math.PI/song.beat))*8;
  if(charState==='jump'){
    jumpT+=dt; const p=Math.min(1,jumpT/0.42);
    y=gy-Math.sin(p*Math.PI)*H*0.20; key=char.jump; rot=char.jumpRot||0;
  ...
  } else if(char.style==='slide'){
    y = gy; rot = Math.sin(t*3.1)*0.02;
  } else if(char.style==='float'){
    y = gy - Math.sin(t*2.2)*7; rot = Math.sin(t*1.6)*0.04;
  } else {
    y = gy - bob*0.55;
  }
  if(combo>=20 && charState!=='fall'){            // 푸슝 자국
    g.fillStyle='rgba(217,196,160,.5)';
    for(let i=0;i<3;i++){
      const px=charX()-52-i*26-(t*90)%18, r=12-i*3;
      g.beginPath(); g.ellipse(px,y-scale*0.28,r*1.5,r,0,0,6.3); g.fill();
    }
  }
  const img=IMG[key];
  if(!img||!img.complete||!img.naturalWidth){ wobble(charX(),y-scale*0.4,scale*0.4,1,'#FFFDF6',th.ink); return; }
  const w=sc*(img.naturalWidth/img.naturalHeight);
  g.save(); g.translate(charX(),y); g.rotate(rot);
...
function drawHud(t,th){
  const pad=22,y=22,w=W-pad*2-26;
  ...
  if(combo>1){
    ...
    g.font='700 '+Math.min(58,30+combo*0.35)+'px "Gaegu", sans-serif';
    g.save(); g.translate(W*0.5,H*0.21); g.rotate(-0.04); g.fillText(combo,0,0);
  ...
  if(t<song.barDur*2){
    ...
    g.fillText(n<=4?String(n):'준비',W/2,H*0.52); g.globalAlpha=1;
  }
}
...
function frame(ts){
  ...
  if(playing){
    ...
    for(const n of notes){
      ...
      if(n.holding && t>=n.t+n.dur){
        ...
        fx.push({x:judgeX(),y:laneY('hold'),r:12,life:1,kind:'perfect'});
        texts.push({x:judgeX(),y:laneY('hold')-30,life:1,s:'쓰왜!'});
        blip(980,0.1);
      }
      if(n.holding && !holdKey && t<n.t+n.dur-0.12){                     // 놓으면 그 자리에서 끝
        n.holding=false; n.done=true;
        texts.push({x:judgeX(),y:laneY('hold')-26,life:1,s:'놓쳤다'});
        for(let i=0;i<4;i++) fx.push({x:judgeX(),y:laneY('hold'),life:.7,kind:'dust',
  ...
  if(playing||frozen){
    judgeMarks(t,th); drawNotes(t,th); drawChar(t,playing?dt:0,th); drawFx(playing?dt:0,th); drawHud(t,th);
  } else {
