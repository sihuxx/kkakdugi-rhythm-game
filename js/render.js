...
function laneField(t,th){
  ...
  for(let i=0;i<4;i++){
    ...
    if(laneFlash[i]>0.02){                       // 눌릴 때 바깥 테
      g.globalAlpha=laneFlash[i]*0.8; g.lineWidth=3;
...
function drawFx(dt,th){
  ...
  for(const f of fx){
    ...
    if(f.kind==='shard'){
      ...
      g.lineWidth=2; g.strokeStyle='#2B2B2B'; g.stroke(); g.restore();
    } else if(f.kind==='star'){
      g.save(); g.translate(f.x,f.y); g.rotate(f.rot);
      g.fillStyle='#FFD98A'; g.strokeStyle='#2B2B2B'; g.lineWidth=1.6;
      g.beginPath();
      ...
      g.strokeStyle=f.col; g.lineWidth=3*a+1;
      ...
      g.strokeStyle=f.col||'#7BC47F'; g.lineWidth=3.4;
  ...
  for(const tx of texts){
    ...
    g.textAlign='center'; g.lineWidth=5; g.lineJoin='round';
...
function drawHud(t,th){
  ...
  if(combo>1){
    ...
    g.lineWidth=5; g.lineJoin='round'; g.strokeStyle='#FFFDF6'; g.strokeText(combo,0,0);
